...
//        GuiFPS.NumberSprites[1] = DefaultSpriteUI.AddElement(new Vector2(GuiFPS.ScreenLeft - GuiFPS.ScreenWidth, GuiFPS.ScreenBottom), GuiFPS.ScreenWidth, GuiFPS.UvHeight, 10, GuiFPS.UvLeft, GuiFPS.UvTop, GuiFPS.UvWidth, GuiFPS.UvHeight);
	...
	public bool isIngameMenuOn{
		get{
			if(IngameMenu!=null){
				return IngameMenu.IsOn && IngameMenu.State==IngameMenu.E_State.IngameMenu;
			}
			else{
				return false;
			}
	...
	public bool isIngameMenuControlsOn{
		get{
			if(IngameMenu==null){
				return IngameMenu.State==IngameMenu.E_State.Controls;
			}
			else{
				return false;
			}
	...
	/// <summary>
	/// 没测试过，不确定isIngameMenuControlsOn==false的情况下，调用会不会异常
	/// 但是本代码已经控制不调用
	/// </summary>
	public void HideIngameMenuControls(){
		IngameMenu.HideControls(false);
	}
	...
	public void PlayWarningSound(){
		Audio.PlayOneShot(SoundDataManager.Instance.KnockDownSound);
	}
